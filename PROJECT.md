...
## Open threads
- **`www.ecosipboard.com` is being retired rather than fixed (decided 2026-08-18).** GitHub will not issue a cert covering `www` while the current apex-only one is valid to Nov 15, so the name is being dropped: delete the `www` CNAME in Hostinger so `www.ecosipboard.com` stops resolving instead of resolving to a certificate warning. The apex is the canonical and advertised address, so nothing of value is lost. `csinfraestructura.com` keeps its working `www`, so **the two sites are deliberately inconsistent** and that is not a bug to 'fix' later. **Why the obvious fixes do not work, so nobody retries them:** the live cert still carries serial `0632B43D…8889` and `notBefore Aug 17 22:23:48 GMT`, identical to the original, proving the unset/re-set at 05:12 UTC produced no new certificate. GitHub reused the existing valid one, almost certainly because the two calls were ~11 seconds apart with an identical domain value. Root cause was ordering: the first request went out at 2026-08-17 23:21 UTC while the domain had no DNS records at all, so there was no `www` to validate. DNS and repo config are ruled out, both zones are structurally identical with no CAA, and both repos use the same branch, path and enforcement. **If `www` is ever wanted back**, it has to be re-added to DNS *before* a genuinely new cert is requested, i.e. after the mid-October renewal, or by setting the custom domain to a different value to force a fresh request.
