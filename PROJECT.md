...
## Open threads
- **`www.ecosipboard.com` is not covered by the TLS certificate.** GitHub issued an apex-only cert because the `www` CNAME was added *after* the certificate request went out. `https://www.ecosipboard.com` fails the handshake with a wrong-principal error, though `http://www...` still 301s to the apex correctly. `csinfraestructura.com` got a cert covering both names because its `www` record existed at request time, which confirms the cause rather than leaving it a guess. Fix: unset then re-set the custom domain on this repo so a fresh cert is requested for both, costing a few minutes of HTTPS downtime. **Deliberately not done on 2026-08-17** because the site had just come back from an outage. GitHub often reissues on its own within 24h, so check `gh api repos/NILS-DIGITAL-COM/022-ecosipboard/pages` and look at `https_certificate.domains` before spending the downtime.
