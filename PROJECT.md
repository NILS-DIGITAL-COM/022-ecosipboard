...
## Open threads
- **`www.ecosipboard.com` is not covered by the TLS certificate, and the standard fix did not work.** GitHub issued an apex-only cert because the `www` CNAME was added *after* the certificate request. `https://www.ecosipboard.com` fails with a hostname mismatch; `http://www...` still 301s to the apex fine. `csinfraestructura.com` got both names because its `www` record existed at request time, which pins the cause to ordering, not configuration: both repos are set identically (same branch, same path, domain from a `CNAME` file). **Tried and did not work (2026-08-18):** re-`PUT`ting the same `cname` (treated as a no-op), and a full unset then re-set of the custom domain, which rebuilt the site but left the cert at `['ecosipboard.com']`. The recycle cost a short outage and bought nothing, so **do not repeat it**. Remaining option is GitHub's own periodic renewal, which should now include `www` since DNS is correct and stable. Check with `gh api repos/NILS-DIGITAL-COM/022-ecosipboard/pages` and read `https_certificate.domains`. If it has not picked up `www` within a week, the fallback is to stop serving `www` at all: drop the `www` CNAME in Hostinger so the name simply does not resolve, rather than resolving to a cert error.
