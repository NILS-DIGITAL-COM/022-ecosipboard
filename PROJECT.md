...
## Open threads
- **`www.ecosipboard.com` is retired, done 2026-08-18.** The `www` CNAME was deleted in Hostinger, so the name returns NXDOMAIN instead of resolving to a certificate warning. Verified authoritative: `www` gone, apex still on all four Pages IPs, both Titan MX and the SPF TXT untouched, and 8/8 requests across all four edge IPs return 200. The apex is the canonical and advertised address so nothing of value was lost. **`csinfraestructura.com` keeps its working `www`, so the two sites are deliberately inconsistent and that is not a bug to 'fix'.** Why the obvious fixes were abandoned, so nobody retries them: the live cert kept serial `0632B43D…8889` and `notBefore Aug 17 22:23:48 GMT` through an unset/re-set, proving GitHub reused the existing cert rather than issuing a new one, because the calls were ~11 seconds apart with an identical value and the old cert is valid to Nov 15. Root cause was ordering: the first request went out while the domain had no DNS records at all. DNS and repo config were both ruled out. **If `www` is ever wanted back**, add it to DNS *before* a genuinely new cert is requested, i.e. after the mid-October renewal, or by setting the custom domain to a different value to force a fresh request.
- **`_dmarc.ecosipboard.com` still contains the template placeholder `rua=mailto:dmarc@yourdomain.com`.** Aggregate reports are being addressed to a domain the client does not own, so nobody is receiving them and the policy is `p=none`, i.e. monitor-only with the monitoring broken. The domain runs real Titan email, so this is worth fixing: point `rua` at a mailbox that exists. `csinfraestructura.com` has no `_dmarc` record at all. Neither is caused by the migration, both were pre-existing and only surfaced while auditing the zone.
